...
class Sa2VAChatModel(PreTrainedModel):
    ...
    def __init__(
        ...
        # Safe template normalization BEFORE using PROMPT_TEMPLATE
        self.template = getattr(config, "template", "qwen_chat") or "qwen_chat"
        self.template = self.template.replace("-", "_")
        from .templates import PROMPT_TEMPLATE as _PT

        if self.template not in _PT:
            logger.warning(f"Template {self.template} not found, fallback to qwen_chat")
        ...
        self.template = self.conv_template  # retain original downstream usage pattern
    ...
    def _llm_forward(
        ...
        vit_embeds = vit_embeds.to(
            input_embeds.dtype
        )  # FIXME: why vit_embeds is float16?
        ...
        if vp_embeds is None:
            try:
                ...
                if n_token > len(vit_embeds):
                    print(
                        f"Wrong !!! {n_token} image tokens in text but only {len(vit_embeds)} vit embeds !!!"
                    )
            ...
            try:
                ...
                if n_token > len(vp_embeds):
                    print(
                        f"Wrong !!! {n_token} image tokens in text but only {len(vp_embeds)} vit embeds !!!"
                    )
    ...
    def predict_forward(
        ...
        if image is None and video is None and "<image>" not in past_text:
            ...
            if video is not None:
                ...
                pixel_values = torch.stack(pixel_values, dim=0).to(
                    self.torch_dtype
                )  # (n_f, 3, h, w)
            ...
            if mask_prompts is not None:
                ...
                mask_prompts = [
                    torch.Tensor(item).to(pixel_values.device) for item in mask_prompts
                ]
                mask_prompts = [
                    F.interpolate(
                        item.unsqueeze(0),
                ...
                vp_token_str = "\nThere are {} part regions in the picture: ".format(
                    len(mask_prompts[0])
                )
