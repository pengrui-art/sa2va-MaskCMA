...
class LisaModel(BaseModel):
    def __init__(
        ...
        if self.mllm.use_llm_lora:
            ...
            self.mllm.model.language_model.base_model.model.model.embed_tokens.requires_grad_(
                True
            )
    ...
    def _generate_and_postprocess_masks(
        self, pred_embeddings, image_embeddings, resize_list=None, orig_size_list=None
    ):
        pred_masks = []
        for i, pred_embedding in enumerate(pred_embeddings):
            ...
            pred_mask = self.grounding_encoder.postprocess_masks(
                ...
                original_size=orig_size_list[i],
            )
    ...
    def state_dict(self, *args, **kwargs):
        ...
        to_return.update(
            {
                ...
                if "grounding_encoder.mask_decoder." in k
            }
        )
        ...
        to_return.update({k: v for k, v in state_dict.items() if "lm_head.weight" in k})
        to_return.update(
            {k: v for k, v in state_dict.items() if "embed_tokens.weight" in k}
        )
        return to_return
    ...
    def compute_loss(self, data, data_samples=None, mode="loss"):
        ...
        seg_token_mask = torch.cat(
            [seg_token_mask, seg_token_mask.new_zeros(seg_token_mask.shape[0], 1)],
            dim=-1,
        )
        ...
        for i in range(bs):
            ...
            accuracy = (
                torch.eq((pred_mask.sigmoid() > 0.5), gt_mask).to(pred_mask).mean()
            )
    ...
    def predict(self, data):
        ...
        last_hidden_states = [
            item[-1] for item in hidden_states[1:]
        ]  # remove input_ids
        ...
        g_pixel_values = torch.stack(
            [
                self.grounding_encoder.preprocess(pixel.to(device))
