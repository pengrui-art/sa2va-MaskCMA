...
from projects.glamm.utils import (
    DEFAULT_IMAGE_TOKEN,
...
# Top-level function to ensure PIL image is RGB; picklable for multiprocessing spawn
def _ensure_rgb(img):
...
class ReferSegmDataset(RefCocoDataset):
    ...
    def _parse_annotations(self, ann_info):
        ...
        index = np.random.choice(
            range(len(instances)), self.num_classes_per_sample, replace=True
        )
        for idx in index:
            inst = instances[idx]
        ...
        ann_info.update(
            {"masks": masks, "conversations": conversation, "image": image_path}
        )
        return ann_info
            ...
            if self.preprocessor is not None:
                if self.arch_type == "qwen":
                    ...
                    _data_dict = self.preprocessor(
                        images, do_resize=True, size=(self.image_size, self.image_size)
                    )
                    ...
                    num_image_tokens = (
                        _data_dict["pixel_values"].shape[0] * self.patch_token
                    )
                else:
        ...
        for msg in conversations:
            if msg["from"] == "human":
                ...
                if "<image>" in msg["value"]:
                    msg["value"] = (
                        msg["value"].replace("<image>", image_token_str).strip()
                    )
