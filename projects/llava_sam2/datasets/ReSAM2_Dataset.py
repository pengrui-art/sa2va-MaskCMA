    ...
    def __getitem__(self, index):
        try:
            ...
            filename = os.path.basename(
                expression_dict["video_path"]
            )  # "sav_031606.mp4"
            dirname = os.path.basename(
                os.path.dirname(expression_dict["video_path"])
            )  # "sav_031"
            correct_subpath = os.path.join(
                dirname, filename
            )  # "sav_031/sav_031606.mp4"
            video_path = os.path.join(self.sam2_folder, correct_subpath)
            # print(
            #     f"Video path: {video_path}    sam2_folder: {self.sam2_folder}     correct_subpath: {correct_subpath}"
            # )
            filename = os.path.basename(
                expression_dict["anno_path"]
            )  # "sav_031606.mp4"
            dirname = os.path.basename(
                os.path.dirname(expression_dict["anno_path"])
            )  # "sav_031"
            correct_subpath = os.path.join(
                dirname, filename
            )  # "sav_031/sav_031606.mp4"
            anno_path = os.path.join(self.sam2_folder, correct_subpath)
            ...
            if video_frames is None:
                raise ValueError(f"video_frames is None for {video_path}")
            ...
            if n_frames > self.sampled_frames + 1:
                if self.frame_contiguous_sample and random.random() < 0.5:
                    ...
                    selected_start_frame = np.random.choice(
                        n_frames - self.sampled_frames, 1, replace=False
                    )
                    selected_frame_indexes = [
                        selected_start_frame[0] + _i
                        for _i in range(self.sampled_frames)
                    ]
                else:
                    selected_frame_indexes = np.random.choice(
                        n_frames, self.sampled_frames, replace=False
                    )
            else:
                selected_frame_indexes = np.random.choice(
                    n_frames, self.sampled_frames, replace=True
                )
            ...
            if self.use_fast:
                ...
                fast_pixel_values = torch.stack(
                    fast_pixel_values, dim=0
                )  # (n_f, 3, h, w)
            ...
            print(f"[CRITICAL ERROR in Dataset] {e}")
