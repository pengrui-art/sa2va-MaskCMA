...
def _get_rawvideo_dec(video_path, select_frames=5):
    ...
    assert (
        num_frames > 0
    ), f"num_frames: {num_frames}, f_start: {f_start}, f_end: {f_end}, fps: {fps}, video_path: {video_path}"
    # T x 3 x H x W
    if num_frames <= select_frames:
        ...
        sample_pos = [
            np.random.randint(split_point[i], split_point[i + 1])
            for i in range(select_frames)
        ]
...
class VideoChatUniViDataset(Dataset):
    ...
    def dataset_map_fn(self, data_dict, select_k=5):
        ...
        if self.use_fast:
            fast_images = _get_rawvideo_dec(
                video_file, select_frames=self.n_fast_images
            )
        else:
            fast_images = None
    ...
    def prepare_text(
        self, n_frames, conversation, num_image_tokens=256, n_fast_images=0
    ):
    ...
    def __getitem__(self, index):
        ...
        data_dict = self.dataset_map_fn(
            selected_data_dict, select_k=self.sampled_frames
        )
